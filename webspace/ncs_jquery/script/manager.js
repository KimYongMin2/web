...
$(document).ready(function() {
        ...
        $('#loginForm').submit(function () {
            ...
            if (id.val().trim().length < 1) {
                $('#idMsg').html('필수항목입니다.').css('display', 'block');
                return false;
            }
            ...
            if (repw.val().trim().length < 1) {
                $('#pw_re+div.msg').html('필수항목입니다.').css('display', 'block');
                return false;
            }

            if (pw.val().trim() != repw.val().trim()) {
                $('#pw_re+div.msg').html('비밀번호가 일치하지않습니다.').css('display', 'block');
                return false;
            }

            if (name.val().trim().length < 1) {
                $('#name+div.msg').html('필수항목입니다.').css('display', 'block');
                return false;
            }

            if (year.val().trim().length < 1) {
                $('#birthwrap + div.msg').html('필수항목입니다.').css('display', 'block');
                return false;
            }

            if (gender.val().trim().length < 1) {
                $('#gender+div.msg').html('필수항목입니다.').css('display', 'block');
                return false;
            }
